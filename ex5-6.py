...
rabbits=[-1 for _ in range(n)]
rabbits[0]=1
rabbits[1]=1

def wabbits(month):

    if(rabbits[month-1]==-1):
        deceased = 0
        if month - k > 0:
            if month == k + 1:
                deceased = 1
            else:
                deceased = wabbits(month - (k + 1))
        #print(rabbits)
        rabbits[month-1]= wabbits(month - 2) + wabbits(month - 1) - deceased

    return rabbits[month-1]



    #result = wabbits(month - 2) + wabbits(month - 1) - deceased
    #print(f"Month {month}:alive {result} deceased {deceased}")
